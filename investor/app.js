...
async function logout() {
  await fetch('/api/auth/investor/logout', { method:'POST' });
  location.assign('/investor/');
}
document.querySelectorAll('.logout-link').forEach((button) => button.addEventListener('click', logout));

try {
  await api('/api/auth/investor/me');
  if (page === 'overview') {
    const data = await api('/api/investor/overview');
  ...
  if (page === 'music') {
    const { tracks } = await api('/api/investor/music');
  ...
  if (page === 'global') {
    const { representatives } = await api('/api/investor/global');
