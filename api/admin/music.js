...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      const slug = hasSlug ? normalizeTrackSlug(body.slug) : null;
      const updated = await query('UPDATE music_tracks SET title = COALESCE($2, title), genre = COALESCE($3, genre), concept = COALESCE($4, concept), target_artist = COALESCE($5, target_artist), description = COALESCE($6, description), display_order = COALESCE($7, display_order), is_active = COALESCE($8, is_active), slug = CASE WHEN $9 THEN $10 ELSE slug END, updated_at = NOW() WHERE id = $1 RETURNING id', [trackId, body.title ?? null, body.genre ?? null, body.concept ?? null, body.targetArtist ?? null, body.description ?? null, Number.isInteger(body.displayOrder) ? body.displayOrder : null, typeof body.isActive === 'boolean' ? body.isActive : null, hasSlug, slug]);
    ...
    if (error.code === 'INVALID_SLUG') return json(res, 400, { error: error.message });
