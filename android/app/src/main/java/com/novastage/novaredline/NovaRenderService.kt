...
class NovaRenderService : Service() {
    ...
    companion object {
        ...
        fun startAuto(context: android.content.Context, sourceUri: Uri) {
            val intent = Intent(context, NovaRenderService::class.java).apply {
                action = ACTION_START
                putExtra(EXTRA_URI, sourceUri.toString())
        ...
        fun cancel(context: android.content.Context) {
            context.startService(Intent(context, NovaRenderService::class.java).apply { action = ACTION_CANCEL })
        }
    }
    ...
    private fun startRender(intent: Intent) {
        ...
        val source = intent.getStringExtra(EXTRA_URI)?.let(Uri::parse) ?: return stopSelfSafely()
        ...
        renderJob = scope.launch {
            ...
            try {
                ...
                if (autoProfile != null) {
                    NovaRenderBus.emit(NovaRenderState.Running(.30f, "AUTO PROFILE · ${autoProfile.name}"))
                    updateNotification("AUTO PROFILE · ${autoProfile.name}", 30)
                }

                val result = engine!!.renderMaster(prepared, renderParams, target, ceiling) { p, s ->
                ...
                NovaRenderBus.emit(NovaRenderState.Failed(t.message ?: t.toString()))
    ...
    private fun startForegroundNow(text: String, progress: Int) {
        ServiceCompat.startForeground(this, NOTIFICATION_ID, buildNotification(text, progress), ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROCESSING)
    }
        ...
        NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .addAction(
                ...
                PendingIntent.getService(
                    this, 10,
                    Intent(this, NovaRenderService::class.java).apply { action = ACTION_CANCEL },
    ...
    private fun createChannel() {
        getSystemService(NotificationManager::class.java).createNotificationChannel(
            NotificationChannel(CHANNEL_ID, "NOVA Master Rendering", NotificationManager.IMPORTANCE_LOW).apply {
    ...
    private fun persistLastResult(result: NovaMasterResult) {
        getSharedPreferences("nova_last_master", MODE_PRIVATE).edit()
    ...
    override fun onTimeout(startId: Int, fgsType: Int) {
        renderJob?.cancel()
        NovaRenderBus.emit(NovaRenderState.Failed("Android mediaProcessing foreground-service timeout"))
        stopForeground(STOP_FOREGROUND_REMOVE)
        stopSelf()
    }

    override fun onDestroy() {
        renderJob?.cancel(); scope.cancel(); super.onDestroy()
    }

    private fun stopSelfSafely(): Int { stopSelf(); return START_NOT_STICKY }
}
